...
**Last Updated:** 2026-09-02 (cont.16)
**Design docs (current):** GDD v6.7.0 (paired-down hub) · Removed-Content Archive v1.3.0 · The Why v2.3.0 · Build v1.5.0 · Deploy v1.4.0 · Shape Catalog v1.0.0 · **Cannons & Towers v2.4.0** · **Ships v2.4.0** · Fleet Memory v2.3.0 · Campaign v2.3.0 · **Victory v2.4.0** · Fight v1.5.0 · Mini-Games v0.1.0. Governed by The Why. **Every HTML doc single-sources its own version** — one `--doc-version` in `:root`, rendered via `<span class="ver"></span>`. **Design detail lives in the module docs — this journal holds the control panel, the current design canon, and session records.**
...
|---|---|---|---|---|
| 1 | **Formation composition** — which lineages fill core vs screen slots, ratios, how many screen slots a core type carries, multi-core shapes | Feeds queue depth, which feeds sortie count, which feeds the race. Upstream of everything below. | journal + Ships + Victory + Fleet Memory | Opus, high |
| 2 | **The elimination rule** — when does a formation count as finished? (all hulls dead / core dead / core dead + orphans absorbed) Plus the revenge state and the sortie-refusal threshold | A real design question, not a number. Sets the elimination rate, and therefore sortie count. Needs #1 first. | journal + Ships + Victory + Fleet Memory | Opus, high |
| 3 | **The falloff rebuild** — edge floor, Coordination increment, mean P(hit) | Feeds k₁'s bottom-up cross-check, and now also feeds *how much* a screen leaks (cont.16), so it is load-bearing on screening too. | journal + Cannons + Victory | Opus, medium |
