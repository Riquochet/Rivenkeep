...
**Last Updated:** 2026-09-02 (cont.12)
**Design docs (current):** GDD v6.7.0 (paired-down hub) · Removed-Content Archive v1.3.0 · The Why v2.0.5 · **Build v1.2.0** · **Deploy v1.2.0** · Shape Catalog v1.0.0 · **Cannons & Towers v2.1.0** · Ships v2.0.1 · Fleet Memory v2.0.4 · Campaign v2.0.1 · Victory v2.0.5 · **Fight v1.2.1** · Mini-Games v0.1.0. Governed by The Why. **Design detail lives in the module docs — this journal holds the control panel, the current design canon, and session records.**
...
0. **FLEET SIZING + THE CANNON ECONOMY — the upstream blocker (Jack, cont.12; economy explicitly tabled here cont.12b).** How many ships a battle fields, expressed as **peak simultaneous contacts per approach** (pool size falls out of that, not the reverse). *Everything numeric downstream waits on it:* cannon slot counts, the wall budget, and all three phase timers. Run **against legibility, not difficulty** — the ceiling is The Why §6's single-point-of-failure. Docs: Ships, Fleet Memory, Victory, Cannons.
0b. **ALL THREE PHASE TIMERS — one dedicated session (Jack, cont.12).** Build + Deploy + Fight together, gated on #0, **calibrated against Rampart** (the acknowledged benchmark for Build-phase timing). Deploy's 12–15s is void; Build's 25→20s and Fight's 90s were both set under the old ship counts and old targeting model. Docs: Build, Deploy, Victory, Fight.
